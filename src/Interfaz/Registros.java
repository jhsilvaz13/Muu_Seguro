...
public class Registros extends javax.swing.JFrame {
    ...
    private void initComponents(Empresa emp) {
        ...
        cal.add(cal.YEAR, -30);
        jDateChooserFechaNacimiento.getJCalendar().setMinSelectableDate(cal.getTime());
        ...
        jPanelRegistroAnimal.add(jLabeCantidadLeche, new org.netbeans.lib.awtextra.AbsoluteConstraints(55, 444, 134, -1));
        ...
        jPanelRegistroAnimal.add(jTextFieldCantidadLeche, new org.netbeans.lib.awtextra.AbsoluteConstraints(55, 460, 192, 30));
        ...
        jSpinnerEdad.setModel(new javax.swing.SpinnerNumberModel(1, 1, 30, 1));
    ...
    */
    private void jButtonRegistrarAnimalActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonRegistrarAnimalActionPerformed
        ...
        if(jRadioButtonNacimientoSI.isSelected()){
            edad=calcularEdad();
            fechaNacimiento=new SimpleDateFormat("dd/MM/yyyy").format(jDateChooserFechaNacimiento.getDate());
        }else{
            if(sexo.equals("Vaca") || sexo.equals("Toro")){
                edad=(Integer)jSpinnerEdad.getValue();
                fechaNacimiento=calcularFechaNacimiento(edad);
            }else{
                edad=0;
                fechaNacimiento=calcularFechaNacimiento(edad);
            }
        ...
        if (noEspaciosEnBlanco(numRegistro) && noEspaciosEnBlanco(""+peso)
            ...
            if(!numCrias.equals("-1")){                         
                if(((jRadioButtonNacimientoSI.isSelected() || jRadioButtonNacimientoNO.isSelected()) && (jRadioButtonCriasSI.isSelected() || jRadioButtonCriasNO.isSelected())) || (String.valueOf(jComboBoxTipoSexo.getSelectedItem()).compareTo("Ternero")==0 || String.valueOf(jComboBoxTipoSexo.getSelectedItem()).compareTo("Ternera")==0)){
                    if(comprobarSiAnimalNoExiste(sexo, numRegistro)){
                        empresa.darAdmin().registrarAnimal(numRegistro, nombreAnimal,raza,sexo, fechaNacimiento,edad,Float.parseFloat(peso),Integer.parseInt(numCrias),Float.parseFloat(cantidadCarne),Float.parseFloat(cantidadLeche));
                        limpiarPaneles();
                    }else{
                        ...
                        if(respuesta!=-1 && respuesta!=2){
                            if(respuesta==0){
                                //Modificar animal
                                empresa.darAdmin().modificarAnimal(numRegistro, nombreAnimal,raza,sexo,fechaNacimiento,edad,Float.parseFloat(peso),Integer.parseInt(numCrias),Float.parseFloat(cantidadCarne),Float.parseFloat(cantidadLeche));
                                limpiarPaneles();
                            }else if(respuesta==1){
                                //Eliminar animal
    ...
    /*
    *Componentes correspondientes al tipo de sexo
    */
    private void jComboBoxTipoSexoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jComboBoxTipoSexoActionPerformed
        Date hoy = new Date();
        java.util.Calendar cal = java.util.Calendar.getInstance();
        if(((String)jComboBoxTipoSexo.getSelectedItem()).equals("Ternero") || 
            ((String)jComboBoxTipoSexo.getSelectedItem()).equals("Ternera")){      
           cal.add(cal.YEAR, -1);
           jTextFieldCantidadLeche.setVisible(false);
           jLabeCantidadLeche.setVisible(false);
           jRadioButtonCriasSI.setVisible(false);
           jRadioButtonCriasNO.setVisible(false);
           jTextFieldCrias.setVisible(false);
           jLabelNumCrias.setVisible(false);
           jSpinnerEdad.setVisible(false);
       }else if(((String)jComboBoxTipoSexo.getSelectedItem()).equals("Toro")){
           cal.add(cal.YEAR, -30);
           jTextFieldCantidadLeche.setVisible(false);
           jLabeCantidadLeche.setVisible(false);
           jRadioButtonCriasSI.setVisible(true);
           jRadioButtonCriasNO.setVisible(true);
           jTextFieldCrias.setVisible(true);
           jLabelNumCrias.setVisible(true);
    ...
    */
    private void jButtonRegistroUbicacionActionPerformed(java.awt.event.ActionEvent evt){
        ...
        tab=new Tabla_Lotes(this, empresa, width, height, true, true);
    }
    ...
    */
    private boolean comprobarSiAnimalNoExiste(String sexo, String numeroReg){
        ...
        }else if(sexo.equals("Ternero")){
             if(empresa.darAdmin().comprobarSiTerneroExiste(numeroReg)){
                return false;
            }else{
                return true;
            }
        }else{
             if(empresa.darAdmin().comprobarSiTerneraExiste(numeroReg)){
                return false;
            }else{
                return true;
            }
    ...
    */
        ...
        if(jRadioButtonCriasSI.isSelected()){
            if(noEspaciosEnBlanco(jTextFieldCrias.getText())){
                ...
                for(int i=0; i<numerosSerie.length; i++){
                    if(!comprobarSiAnimalNoExiste("Ternero",numerosSerie[i]) || !comprobarSiAnimalNoExiste("Ternera",numerosSerie[i])){
                        aux+=1;
                    }else{
                        return "-1";
                    }  
    ...
    private String calcularFechaNacimiento(int edadMeses){
        Date hoy = new Date();
        java.util.Calendar cal = java.util.Calendar.getInstance();
        cal.setTime(hoy);
        cal.add(cal.MONTH, -(edadMeses));
        String nacimiento= new SimpleDateFormat("dd/MM/yyyy").format(cal.getTime());
        return nacimiento;
    }
